...
const salvarOuAtualizar = async (alunoId, peso, altura) => {
  ...
  // calcula o IMC manualmente não dependendo do trigger
  const imcCalculado = (parseFloat(peso) / (parseFloat(alturaM) * parseFloat(alturaM))).toFixed(2);
  ...
  if (existe.length > 0) {
    await pool.query(
      ...
      [peso, alturaM, imcCalculado, alunoId]
    );
  } else {
    await pool.query(
